...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		ifstream framesFile(framesFilePath.c_str());
		...
		imgFormat = "/home/kris-allen/AIBC/github/OTB/tracker/struck/" + conf.sequenceBasePath+"/"+conf.sequenceName+"/img/%d.jpg";
		...
		string gtFilePath = "/home/kris-allen/AIBC/github/OTB/tracker/struck/" + conf.sequenceBasePath+"/"+conf.sequenceName+"/"+"init_rect.txt";
		// ifstream gtFile(gtFilePath.c_str(), ios::in);
		ifstream gtFile(gtFilePath.c_str());
		if (!gtFile)
		{
	...
	for (int frameInd = startFrame; frameInd <= endFrame; ++frameInd)
	{
		...
		if (tracker.IsInitialised())
		{
			...
			if (outFile)
			{
				...
				//output is the xmin, ymin, width, height
